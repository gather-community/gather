...
Gather.Views.AjaxSelect2 = Backbone.View.extend({
  ...
  setupSelect2($select) {
    $select.select2({
      ...
      language: { inputTooShort() { return $select.data('select2-prompt'); }
    },
  ...
  buildGetParams(params, $select) {
    ...
    if (typeof(extraData) === 'function') { extraData = extraData.call(); }
    if (extraData instanceof Array) { extraData = {data: JSON.stringify(extraData)}; }
