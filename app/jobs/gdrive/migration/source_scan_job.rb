    ...
    class SourceScanJob < ScanJob
      def process_file(gdrive_file)
        ...
        if gdrive_file.id == operation.src_folder_id
          scan.log(:info, "Item is operation src folder, skipping")
          return
        end
      ...
      def process_new_folder(gdrive_file)
        scan.log(:info, "Processing new folder", src_id: gdrive_file.id)
        begin
          ...
          scan.log(:error, "Ancestor inaccessible", file_id: gdrive_file.id, folder_id: error.folder_id)
          ...
          scan.log(:error, "Client error ensuring tree", file_id: file_id, message: error.to_s)
      ...
      def process_existing_folder(folder_map, gdrive_file)
        ...
        if gdrive_file.trashed
          scan.log(:info, "Folder is in trash, deleting folder map", src_id: folder_map.src_id, dest_id: folder_map.dest_id)
          folder_map.destroy
          return true
        end
        ...
        if new_src_parent_id != old_src_parent_id
          ...
          begin
            ...
            scan.log(:error, "Ancestor inaccessible, skipping", src_id: folder_map.src_id, dest_id: folder_map.dest_id, ancestor_id: error.folder_id)
            return false
          rescue Google::Apis::ClientError => error
            scan.log(:error, "Client error ensuring tree, skipping", src_id: folder_map.src_id, dest_id: folder_map.dest_id, message: error.to_s)
            return false
          end
        ...
        begin
          ...
          scan.log(:info, "Updating dest folder", src_id: folder_map.src_id, dest_id: folder_map.dest_id)
          ...
          scan.log(:error, "Client error updating dest folder", src_id: folder_map.src_id, dest_id: folder_map.dest_id, message: error.to_s)
          return false
        end
        true
      end
      ...
      def process_new_file(gdrive_file)
        scan.log(:info, "Processing new file", id: gdrive_file.id)

        if gdrive_file.trashed
          scan.log(:info, "File is in trash, skipping", file_id: gdrive_file.id, name: gdrive_file.name)
          return
        end

        if gdrive_file.parents.nil?
          scan.log(:info, "File has no parents, skipping", file_id: gdrive_file.id, name: gdrive_file.name)
          return
        end

        scan.log(:info, "File not found, creating", file_id: gdrive_file.id, name: gdrive_file.name)
      ...
      def process_existing_file(gdrive_file, migration_file)
        scan.log(:info, "Processing existing file", file_id: gdrive_file.id)
        ...
        if migration_file.migrated?
          scan.log(:info, "File has been migrated, skipping", file_id: gdrive_file.id, status: gdrive_file.status)
          return
        end

        # parents.nil? means the item is no longer accessible.
        if gdrive_file.trashed || gdrive_file.parents.nil?
          scan.log(:info, "File is in trash, deleting record", file_id: gdrive_file.id)
          migration_file.update!(status: "disappeared")
          return
        end

        scan.log(:info, "Updating record", file_id: gdrive_file.id)
      ...
      def add_file_error(migration_file, type, message)
        scan.log(:error, "File error", id: migration_file.id, type: type, message: message)
