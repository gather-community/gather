...
module Stripe
  ...
  # receipt, before processing. Deliberately NOT tenant-scoped: the cluster isn't known until
  # processing resolves the subscription, and some events never map to a subscription.
  #
  # Note: payloads may contain customer PII (email, address). They live only in our DB, never
  # in logs or Sentry. This class reopens the stripe gem's Stripe namespace.
  class WebhookEvent < ApplicationRecord
    self.table_name = "stripe_webhook_events"

    validates :event_id, :event_type, :payload, presence: true
