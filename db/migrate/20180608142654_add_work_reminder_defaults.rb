...
class AddWorkReminderDefaults < ActiveRecord::Migration[5.1]
  def up
    execute("INSERT INTO work_reminders (cluster_id, job_id, rel_time, time_unit, created_at, updated_at)
      SELECT cluster_id, id, -1, 'days', NOW(), NOW() FROM work_jobs WHERE title NOT IN
