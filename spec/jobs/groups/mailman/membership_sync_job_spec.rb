...
describe Groups::Mailman::MembershipSyncJob do
  ...
  context "when all_cmty_members_can_send is true" do
    # Tests that community members added as nonmembers via the feature
    # don't appear as additional_senders on the group page.
    let!(:community_user) { create(:user, email: "community@example.com") }
    let!(:mm_list) { create(:group_mailman_list, remote_id: "list3.blah", all_cmty_members_can_send: true) }
    let!(:community_mm_user) { build(:group_mailman_user, user: community_user, remote_id: nil) }
    let(:local_mships) do
      [
        # This community user's nonmember entry is generated by community_sender_memberships.
        # Since it's in local, it should not appear in missing_on_local → not in additional_senders.
        build_mship(mailman_user: community_mm_user, list_id: "list3.blah",
          role: "nonmember", moderation_action: "accept")
      ]
    end
    let(:remote_mships) do
      [
        # Matches local exactly — should be left alone and NOT added to additional_senders.
        build_mship(remote_id: "r11", mailman_user: community_mm_user, list_id: "list3.blah",
          role: "nonmember", moderation_action: "accept")
      ]
    end
    subject(:job) { described_class.new("Groups::Mailman::List", mm_list.id) }
    ...
    it "does not add community sender nonmembers to additional_senders" do
      perform_job
      mm_list.reload
      expect(mm_list.additional_senders).to be_empty
    end
  end
