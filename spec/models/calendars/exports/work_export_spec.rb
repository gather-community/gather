  ...
  context "your jobs" do
    ...
    it "includes all meal_assignments and work_assignments" do
      ...
      expect_events({
        ...
        "DTSTART;VALUE=DATE" => period_start.to_date.to_s(:iso8601_no_sep),
        "DTEND;VALUE=DATE" => (period_start.to_date + 1).to_s(:iso8601_no_sep)
      }, {
        uid: "#{signature}_Work_Assignment_#{job3.shifts[0].assignments[0].id}",
        summary: "Multi-day (End)",
        location: nil,
        description: %r{Do something periodically\s+\n http://.+/work/signups/},
        "DTSTART;VALUE=DATE" => period_end.to_date.to_s(:iso8601_no_sep),
        "DTEND;VALUE=DATE" => (period_end.to_date + 1).to_s(:iso8601_no_sep)
      }, {
        summary: "Single-day",
        location: nil,
        description: %r{A very silly job\.\s+\n http://.+/work/signups/},
        "DTSTART;VALUE=DATE" => shift2_1_start.to_date.to_s(:iso8601_no_sep),
        "DTEND;VALUE=DATE" => (shift2_1_start.to_date + 1).to_s(:iso8601_no_sep)
      }, {
        ...
        "DTSTART;TZID=Etc/UTC" => (meal1_time - 2.hours).to_s(:iso8601_no_sep),
        "DTEND;TZID=Etc/UTC" => meal1_time.to_s(:iso8601_no_sep)
      }, {
        uid: "#{signature}_Work_Assignment_#{job1.shifts[1].assignments[0].id}",
        summary: "Assistant Cook: Buns",
        location: meal2.resources[0].name,
        description: %r{Help cook the things\s+\n http://.+/work/signups/},
        "DTSTART;TZID=Etc/UTC" => (meal2_time - 2.hours).to_s(:iso8601_no_sep),
        "DTEND;TZID=Etc/UTC" => meal2_time.to_s(:iso8601_no_sep)
      }, {
        ...
        "DTSTART;TZID=Etc/UTC" => (meal3_time - 90.minutes).to_s(:iso8601_no_sep),
        "DTEND;TZID=Etc/UTC" => meal3_time.to_s(:iso8601_no_sep)
      }, {
        ...
        "DTSTART;VALUE=DATE" => meal4_time.to_date.to_s(:iso8601_no_sep),
        "DTEND;VALUE=DATE" => (meal4_time.to_date + 1).to_s(:iso8601_no_sep)
      })
    end
  end
end
