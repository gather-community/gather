  ...
  describe "populate!" do
    it "should populate properly" do
      item1 = create(:line_item, account: account, incurred_on: "2015-01-01",
        amount: 1.23, invoice: create(:invoice, account: account))
      ...
      invoice = Invoice.new(account: account, prev_balance: -0.12)
    ...
    it "should raise if there are no relevant line items and balance is zero" do
      item1 = create(:line_item, account: account, incurred_on: "2015-01-01",
        amount: 1.23, invoice: create(:invoice))
      invoice = Invoice.new(account: account, prev_balance: 0)
