...
describe Lens::Storage do
  ...
  describe "initial state" do
    it "is correct" do
      storage = Lens::Storage.new(session: session, community_id: 3, controller_path: "a/b", action_name: "c", persist: true)

      expect(session[:lenses]["V"]).to eq(ver)
      expect(session[:lenses]["T"]).to be > 10000000
  ...
  describe "reset param" do
    let(:session) { {lenses: {"V" => ver, "T" => 12345678, "3" => {"a/b__c" => {}}}, otherkey: 1} }

    it "clears old values" do
      Lens::Storage.new(session: session, community_id: 3, controller_path: "a/b", action_name: "c", persist: true, reset: true)
  ...
  describe "reset on old version" do
    let(:session) { {lenses: {"V" => 2, "T" => 12345678, "3" => {"a/b__c" => {}}}, otherkey: 1} }
  ...
  describe "expiry" do
    it "clears lenses on init if expiry time has elapsed" do
      ...
      t1 = session[:lenses]["T"]
      expect(t1).to be > 10000000

      # Not cleared after one minute
      Timecop.freeze(Time.current + 1.minute) do
        Lens::Storage.new(session: session, community_id: 3, controller_path: "a/b", action_name: "c", persist: true)
